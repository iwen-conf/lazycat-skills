...
- docs: 新增安装与业务能力上架门禁，要求提审前使用最终 `.lpk` 在真实微服安装成功，并验证核心业务能力正常，失败或未验证不得上架。
- docs: 新增英文应用上架页描述门禁，要求英文应用提审时在开发者中心上架页面补齐英文描述，且不能用 manifest/package 元数据替代。
- docs: 新增商店上架默认桌面端规则，要求默认只选择桌面端、提供 2-5 张真实截图，并在 `package.yml.unsupported_platforms` 声明 `ios`、`tvos`、`android`。
- docs: 为所有 Lazycat 技能入口补充懒猫微服、应用商店和开发者中心凭据环境变量约定，并明确禁止回显或落盘明文凭据。
