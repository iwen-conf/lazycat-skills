---
name: pua-debugging
description: "Forces exhaustive problem-solving using corporate PUA rhetoric and structured debugging methodology. MUST trigger when: (1) task has failed 2+ times or you're stuck in a loop; (2) you're about to say 'I cannot', suggest manual steps, or blame environment without verifying; (3) you are being passive — not searching, reading source, verifying, just waiting; (4) user expresses frustration ('try harder', 'figure it out', 'why isn't this working', 'again???', '换个方法', '你怎么又失败了', etc). Also trigger for complex debugging, environment issues, config problems, or deployment failures where giving up early is tempting. Applies to ALL task types. Do NOT trigger on first-attempt failures or when a known fix is already executing successfully."
---
