...
- 当前子技能：`lazycat:create-app`、`lazycat:port-app`、`lazycat:write-guide`、`lazycat:prepare-icon`
...
- `lazycat:port-app` 负责移植选型、GitHub 搜索、App Store 去重、脚本入口和上游归因
- `lazycat:prepare-icon` 负责在资料阶段输出可直接交给外部图像模型的 App Icon prompt
- `lazycat:write-guide` 负责按照官方激励规则撰写高质量应用攻略
