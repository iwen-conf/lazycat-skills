...
参考 `cloudmaze` 的约定，所有项目必须补齐：

- 根目录 `build.sh`
- 根目录 `Makefile`

**Makefile 必须包含以下标准 Target：**
- `make build`: 负责调用 `lzc-cli project build`。
- `make install`: 负责本地安装验证。**用户必须在安装后真实体验应用核心功能。**
- `make verify`: 串联单元测试、构建和 E2E。
- `make release-prep`: 自动化生成提审所需的截图和测试报告。

**强制执行：严禁在未执行 `make install` 且未真实体验的情况下提交审核。**
详情参考 [references/command-conventions.md](../lazycat:port-app/references/command-conventions.md)。
