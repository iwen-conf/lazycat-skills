...
description: 面向 Lazycat 新项目创建和项目基线统一的 skill。只要用户提到从 0 创建懒猫应用、初始化项目、脚手架、项目标准、Go 后端、Vue、Element Plus、登录、注册、JWT、access_token、refresh_token、无感刷新、认证改造、现金激励、对接微服账户系统、网盘右键菜单、把现有项目补成统一基线等请求，就必须使用此 skill。负责把项目创建阶段收敛到统一规范：Go + Vue + Element Plus，默认具备登录、注册、双 token 和无感刷新，并在用户以现金激励为目标时优先满足官方激励门槛。
---
...
- **Outputs**: 项目基线摘要、技术栈与目录建议、认证方案、激励资格路径、必须完成的模块清单、后续交给 `lazycat:ship-app` 的发布准备接口
- **Quality Gate**: 最终方案必须明确 Go 后端、Vue + Element Plus 前端、登录/注册、`access_token + refresh_token`、无感刷新，以及在激励模式下的账号可获得性、应用类型合规性和附加对接机会
