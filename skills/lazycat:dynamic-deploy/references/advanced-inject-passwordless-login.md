...
迁移第三方应用时，如果上游支持通过启动命令、CLI、环境变量或管理 API 创建用户，推荐先在服务启动后创建固定初始账号，再用本节三阶段联动学习后续改密结果。这样可以做到“不修改业务代码”的可修改式免密登录。

固定初始凭据必须在 README、商店使用说明或 `package.yml.locales.<lang>.usage` 中明示：
...
启动顺序要求：数据库/缓存健康 -> 主应用健康 -> seed/setup 创建用户成功并写入完成标记 -> 浏览器登录页自动填充。不要用 inject 掩盖后端初始化失败，也不要让 seed 在主应用未 ready 时盲目请求管理 API。
