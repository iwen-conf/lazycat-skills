...
在回答或修改 `lzc-deploy-params.yml`、`lzc-manifest.yml`、Go 模板渲染、`application.injects`、`inject-ctx`、OIDC、HTTP Header 身份或相关兼容性问题前，必须先通过 OpenViking 查询官方文档，不要仅凭本文示例或记忆判断。

```bash
ov find "deploy params manifest render application injects inject ctx OIDC HTTP headers" --uri viking://resources/lazycat-developer-docs -n 5
ov read viking://resources/lazycat-developer-docs/<doc-name>/<doc-name>.md
```

如果 `ov find` 因额度失败，使用 `ov grep "deploy-params"`、`ov grep "injects"` 或 `ov grep "OIDC"` 退回文本检索。本文的 `references/` 只作为工程经验和兼容性策略补充，不能替代官方规范。
...
1. **inject 三阶段联动 (Three-Phase Linkage - 👑 唯一首选方案)**：应用拥有复杂 UI，允许用户在应用内首次创建账号或自主修改密码（如 Jellyfin）。在不修改源码的前提下，你必须监听并联动三个阶段：
   ...
   详细示例先通过 OpenViking 查询 `advanced inject passwordless login Jellyfin`，再结合 `references/advanced-inject-passwordless-login.md` 的本地经验模板执行。
...
在进行打包、构建、配置清单、设置部署参数及免密登录脚本注入时，必须先通过 OpenViking 检索并遵循官方规范：

```bash
ov find "spec build package manifest inject ctx deploy params lpk format" --uri viking://resources/lazycat-developer-docs -n 5
```

重点确认主题：Build Spec、Package Spec、Manifest Spec、Inject Context、Deploy Params、LPK Format。检索后在回复中引用对应 `viking://...` URI，方便用户核验。
