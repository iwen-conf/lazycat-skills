...
5. 原创应用包含 Web、移动端、桌面端或多厂家小程序前端时，除非用户明确指定其他技术，必须联动 `arc:frontend` 并使用对应平台默认栈。
...
4. 如果包含 Web、移动端、桌面端或多厂家小程序前端，调用 `arc:frontend` 确认平台默认栈、页面、状态分层、路由、表单和 token 方案；只有用户明确指定时才记录例外栈。
...
- Web、移动端、桌面端或多厂家小程序前端使用 `arc:frontend` 平台默认栈，或明确记录用户指定的例外技术和边界。
