...
5. 不引入无关技能主题：不单独展开高级路由、图标、后台 UI、攻略、更新、排障等旧入口；这些都只作为当前项目内的必要实现细节。
6. 不写真实微服域名、真实密码、API Key、Token 或内部未公开地址。
...
4. 实现或校准核心业务入口，不做无关页面和装饰性内容。
5. 补齐登录/注册、初始化数据、持久化目录、健康检查和文件选择能力。
6. 本地构建并验证安装路径；满足后交给 `lazycat:ship-app`。
