...
For Lazycat platform facts, query OpenViking before setting the baseline: MicroServer account integration, OIDC, HTTP header identity, file association, AI Pod / `AI App` / AI Browser Extension structure, manifest fields, deploy params, and store incentive or review rules.

```bash
ov find "OIDC HTTP headers file association manifest deploy params store rules" --uri viking://resources/lazycat-developer-docs -n 5
ov find "AI Pod AI App browser extension package structure" --uri viking://resources/lazycat-aipod-docs -n 5
```

Local `references/` files define this skill's project baseline and templates; they are not a substitute for official docs.
...
- Frontend has React but is missing shadcn/ui, Zustand, TanStack Query, route guards, or silent refresh.
...
Determine if it's a new project, an update, or auth-only. Assess existing stack and structure. Identify if it's original/ported and if AI is needed. **Establish `package.yml` early** with the unique package ID, version (strictly formatted as `x.x.x`), and name.
