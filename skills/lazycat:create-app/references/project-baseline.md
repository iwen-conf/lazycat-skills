...
- **Backend:** Go
- **Frontend:** use the required stack exactly as specified — React + Vite + Tailwind CSS + shadcn/ui + Zustand + TanStack Query + React Router + React Hook Form + Zod + Framer Motion.
- **Theme:** pick one of the five approved low-saturation palettes (① Slate, ② Warm Sand, ③ Dark Slate, ④ Sage, ⑤ Rose Mist) and apply it via shadcn CSS variables; do not hand-roll hex colors inside components.
...
- Keep the frontend stack aligned with the required baseline: React + Vite + Tailwind CSS + shadcn/ui + Zustand + TanStack Query + React Router + React Hook Form + Zod + Framer Motion.
