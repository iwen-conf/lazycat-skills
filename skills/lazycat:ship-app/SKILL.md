...
- 最终 `.lpk` 已在真实懒猫微服安装成功，应用可打开，核心业务能力和关键用户流程已验证正常；未验证或异常时必须停止提审。
...
7. 使用最终 `.lpk` 安装到真实懒猫微服，确认安装成功、应用可打开、启动和登录正常。
8. 验证核心业务能力和关键用户流程，包括创建/读取/更新/删除等应用主要功能、持久化、文件能力、卸载/升级风险；失败或未验证时停止提审。
9. 准备上架平台选择、2-5 张截图、图标、描述、测试账号、复现步骤、限制说明和 LPK 信息；默认只选择桌面端。
10. 桌面端默认上架时，确认 `package.yml.unsupported_platforms` 包含 `ios`、`tvos`、`android`。
11. 如果应用运行界面或主要用户文案为英文，提交前确认开发者中心上架页面已填写英文描述；不要把 manifest/package 本地化当作已完成上架页英文描述。
12. 若项目来源为迁移，提交前确认开发者中心未勾选“应用程序为本人原创开发或本人是源作者”，并已填写原作者名称和源项目或代码地址。
13. 提交开发者中心；记录版本、时间、状态和证据。
14. 审核通过后验证商店可见性和安装版本；审核失败按问题归类修复。
