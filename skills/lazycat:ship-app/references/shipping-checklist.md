...
- [ ] **Passwordless Login**: Application MUST support passwordless auto-login (免密登录) via OIDC or Inject (`builtin://simple-inject-password` / custom scripts) to ensure a seamless, weak-perception login state. Query OpenViking for `advanced inject passwordless login` before deciding the implementation path.
